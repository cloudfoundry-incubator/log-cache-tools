...
var _ = Describe("TLS", func() {
	...
	Context("NewMutalTLSConfig", func() {
		...
		It("returns an error when given invalid cert/key paths", func() {
			_, err := config.NewMutualTLSConfig("", "", caCertFilename, "")
		...
		It("returns an error when given invalid ca cert path", func() {
			_, err := config.NewMutualTLSConfig(clientCertFilename, clientKeyFilename, "/file/that/does/not/exist", "")
			Expect(err).To(HaveOccurred())
		...
		It("returns an error when given invalid ca cert file", func() {
			...
			_, err := config.NewMutualTLSConfig(clientCertFilename, clientKeyFilename, empty, "")
			Expect(err).To(HaveOccurred())
		...
		It("returns an error when the certificate is not signed by the CA", func() {
			_, err := config.NewMutualTLSConfig(clientCertFilename, clientKeyFilename, nonSignedCAFilename, "")
			Expect(err).To(HaveOccurred())
